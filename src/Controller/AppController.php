...
//        var_dump($idioma);exit;
    ...
    public function recordActivity(){
        ...
        if(($model=='PublicationAdoption'||$model=='AnimalAdoption'||$model=='AnimalShelter')&& str_contains($action,'view')||$model=='Animal' && str_contains($action,'view')||($model=='Pages')){
            ...
            if($model=='Animal' && str_contains($action,'view')){
                ...
                //Obtengo datos para el csv
                if(($model=='PublicationAdoption'||$model=='AnimalAdoption'||$model=='AnimalShelter')&& str_contains($action,'view')){//interacciones con animal
                    $id=$pages[3];
                ...
                }elseif($model=='Pages'){//Para que aparezca en el csv al meno una vez cada usuario
                    $animalspecie='';
                    $animalchip='';
                    $animalsex='';
                    $animalrace='';
            ...
            if($allAddressIDCountry!=NULL){//comprobar que no es test
    ...
    public function numeroMensajes(){//num mensajes del usuario sin leer para mostrar en header
        if($this->Authentication->getIdentity()){
            $currentLogedID=$this->request->getAttribute('identity')['id'];
    ...
    public function changeLang():void
    {
        ...
        if (I18n::getLocale() !== 'es_ES'){#No esta en español
                ...
                I18n::setLocale('en_US');#Pongo inglés
