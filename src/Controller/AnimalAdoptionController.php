...
class AnimalAdoptionController extends AppController
{
    ...
    public function index()
    {
        if($this->Authentication->getIdentity()){
            ...
            if($currentLogedRole!='admin'){//Es protectora
                ...
                $AnimalShelterUserID=$allAnimalShelter->find()->where(['user_id'=>$currentLogedID])->select('animal_id');//id de los animales adoptados en la protectora
                if($keyAnimal||$keyUsuario){//Es una búsqueda
        
                    $Adopcion_animal = $this->AnimalAdoption->find('all', ['limit' => 200])->where(['animal_id IN'=>$AnimalShelterUserID,
                    ['or'=>[['User.username like'=>'%'.$keyUsuario.'%'],['User.username IS'=>NULL]]],
                    ['or'=>[['Animal.name  like'=>'%'.$keyAnimal.'%'],['Animal.name  IS'=>NULL]]],
                    ]);
                }else{//Todos
                ...
                if($keyAnimal||$keyUsuario){//Búsqueda
    ...
    public function add()
    {
        ...
        $currentUserROLE =$currentUser['role']; //rol usuario logeado

        if ($currentUserROLE=='admin'){ //Usuario logeado es admin, puede añadir cualquiera
            $animal = $this->AnimalAdoption->Animal->find('list', ['limit' => 200])->all();
        }else{//Protectora
            $allAnimalShelter = $this->getTableLocator()->get('AnimalShelter');
            $currentAnimalShelter=$allAnimalShelter->find()->where(['user_id'=>$currentUserID])->select('animal_id'); //lista de entradas de animal de ese usuario
    ...
    public function edit($id = null)
    {
        ...
        $AnimalImage=$allAnimal->find()->where(['id'=>$AnimalID])->select('image')->first();//Se coge la imagen del animal para mostrarla
